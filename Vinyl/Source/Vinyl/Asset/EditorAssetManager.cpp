...
namespace Vinyl {
	...
	static AssetType GetAssetTypeFromFileExtension(const std::filesystem::path& extension)
	{
		...
		{
			VL_CORE_WARN("Could not find AssetType for {}", extension);
	...
	void EditorAssetManager::ImportAsset(const std::filesystem::path& filepath)
	{
		...
		VL_CORE_ASSERT(metadata.Type != AssetType::None, "");
	...
	Ref<Asset> EditorAssetManager::GetAsset(AssetHandle handle)
	{
		...
		else
		{
			...
			{
				// import failed
				VL_CORE_ERROR("EditorAssetManager::GetAsset - asset import failed!");
			}
			m_LoadedAssets[handle] = asset;
		}
		// 3. return asset
	...
	bool EditorAssetManager::DeserializeAssetRegistry()
	{
		...
		{
			VL_CORE_ERROR("Failed to load project file '{0}'\n     {1}", path, e.what());
			return false;
		}
