...
namespace Vinyl {
	...
	Ref<Texture2D> TextureImporter::LoadTexture2D(const std::filesystem::path& path)
	{
		...
		{
			VL_PROFILE_SCOPE("stbi_load - TextureImporter::ImportTexture2D");
		...
		{
			VL_CORE_ERROR("TextureImporter::ImportTexture2D - Could not load texture from filepath: {}",path.string());
			return nullptr;
		}
